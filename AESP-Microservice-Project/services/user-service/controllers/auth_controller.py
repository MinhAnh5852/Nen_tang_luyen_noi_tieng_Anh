...
#     try:
#         # Mentor đăng ký mới phải ở trạng thái pending
...
#         # CHẶN ĐĂNG NHẬP NẾU TÀI KHOẢN KHÔNG ACTIVE
...
#         # Cho phép Admin cập nhật cả status từ giao diện quản lý học viên
#         if data.get('status'):
#             user.status = data.get('status').lower()
#         elif old_role != 'mentor' and new_role == 'mentor':
#             # Nếu người dùng tự nâng cấp lên mentor thì phải chờ duyệt
#             user.status = 'pending'
...
#         # Thông báo cho các service khác (như Mentor Service)
#         send_mq_message({
#             "event": "USER_UPDATED",
#             "user_id": user.id,
#             "username": user.username,
...
# # --- 6. API CẬP NHẬT STATUS NHANH (CHO NÚT GẠT TRÊN UI) ---
