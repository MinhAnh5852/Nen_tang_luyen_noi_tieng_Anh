...
interface Plan {
  ...
  features: string[] | string; // Sửa kiểu dữ liệu để chấp nhận cả mảng
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 1. Lấy danh sách gói từ Subscription Service
        ...
        if (userId && token) {
          // 2. Lấy trạng thái gói hiện tại từ API VERIFY
          ...
          if (statusRes.ok && contentType && contentType.includes("application/json")) {
            ...
            setActivePlanId("free-id-001");
            setCurrentStatusText("Bạn đang dùng gói Miễn phí.");
          }

          // 3. Lấy lịch sử giao dịch từ Payment Service
          ...
          setCurrentStatusText("Vui lòng đăng nhập để xem gói dịch vụ.");
        }
      } catch (error) {
        console.error("Lỗi nạp dữ liệu:", error);
        setCurrentStatusText("Lỗi kết nối máy chủ.");
        setActivePlanId("free-id-001");
      } finally {
        setLoading(false);
      }
    ...
    <div className="container" style={{ marginTop: '100px' }}>
      ...
      <div className="plans-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
        {plans.map(plan => {
          ...
          // Xử lý features an toàn để tránh lỗi .split
          const featuresList = Array.isArray(plan.features) 
            ? plan.features 
            : (typeof plan.features === 'string' ? plan.features.split(',') : []);

          return plan.is_active && (
            <div key={plan.id} className={`plan-card ${plan.badge_text ? 'recommended' : ''}`} style={{ border: isCurrent ? '3px solid #4CAF50' : '1px solid #ddd', padding: '20px', borderRadius: '15px', position: 'relative', backgroundColor: 'white' }}>
          ...
          <table className="data-table" style={{ width: '100%', marginTop: '20px', borderCollapse: 'collapse', backgroundColor: 'white', borderRadius: '10px', overflow: 'hidden' }}>
            <thead>
              <tr style={{ background: '#f8f9fa', textAlign: 'left', borderBottom: '2px solid #dee2e6' }}>
                <th style={{ padding: '15px' }}>Mã giao dịch</th>
            ...
            <tbody>
              {transactions.length > 0 ? transactions.map((tx) => (
                <tr key={tx.id} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '15px' }}>#{String(tx.id).padStart(5, '0')}</td>
                ...
                <tr>
                  <td colSpan={4} style={{ textAlign: 'center', padding: '30px', color: '#888' }}>Bạn chưa có giao dịch nào.</td>
                </tr>
              )}
            </tbody>
          </table>
